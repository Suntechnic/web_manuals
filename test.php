...
<section>
    <h2 title="section h2">Шаблоны:</h2>
    <ul>
        <?if (isset($_GET['template'])):?>
        <li>
            <a href="?">Без шаблона</a>
        </li>
        <?endif;?>
    <?
    $DirPath = $_SERVER["DOCUMENT_ROOT"].'/local/templates';
    $Dir = opendir($DirPath);
    
    while($Node = readdir($Dir)) {
        if (is_dir($DirPath.'/'.$Node) && $Node != '.' && $Node != '..'):?>
        <li>
            <a href="?template=<?=$Node?>" <?if($Node==$_GET['template']):?>style="color:red;"<?endif?>><?=$Node?></a>
        </li>
        <?endif;
    }
    ?>
